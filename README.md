...
dsh 官方默认只允许本机（loopback）访问 web。本插件提供可选的「局域网访问（LAN 绑定）」开关（设置页 → 🔧 功能开关，需重启生效）：把 web 重绑到 `0.0.0.0`，局域网设备可打开界面。

> ⚠️ 注意事项：
> 1. **安全**：0.0.0.0 = 局域网任何设备都能驱动 agent（含 shell），建议在防火墙限制该端口只允许常用设备
> 2. **特权 API**：非本机访问「设置/凭证」等 API 依赖 dsh 的 `trustedHosts` 机制（`dsh-client-connection` 配置）。当前 dsh 官方版本存在已知限制：`trustedHosts` 配置读取后未在围栏生效——若局域网访问时设置/凭证返回 403，属**官方安全拒绝**，不影响其他功能；需要完整支持请自行评估补丁或等待官方修复
> 3. 本功能默认关闭，与官方安全模型一致；不需要请保持关闭
