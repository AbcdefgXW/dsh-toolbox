...
export async function searchSessions(keyword, signal, limit = 100, fromIndex = 0, dateFrom = 0, dateTo = 0, archivedIds = []) {
  ...
  const hits = [];
  // 一次搜全部分组：主会话（visible/archived 按归档状态分）+ 回收站（trash）
  // 子代理会话（header.parentSession 非空）左侧嵌套不可见，直接排除
  const archived = new Set(archivedIds);
  const archivedMarked = new Map(); // sessionId → true（用于标记归档命中）
  const sessions = (await listAllSessions())
    .filter((s) => !(s.header && s.header.parentSession))
    .map((s) => ({ ...s, _bucket: archived.has(s.sessionId) ? "archived" : "visible" }));
  ...
  for (let i = Math.min(fromIndex, sessions.length); i < sessions.length; i++) {
    ...
    try {
      ...
      if (frames.length === 0) {
        // 单帧场景（如插件写回的小文件）：整体解
        hits.push(...searchText(zstdDecompressSync(buf).toString("utf-8"), kw, s, 1, hits.length, limit, dateFrom, dateTo, s._bucket));
      ...
      for (const fr of frames) {
        ...
        hits.push(...searchText(text, kw, s, lineNo, hits.length, limit, dateFrom, dateTo, s._bucket));
        lineNo += text.split("\n").length;
        if (hits.length >= limit) break;
      }
    } catch (err) {
