...
import { listAllSessions, WORKSPACE_ROOT } from "./sessions.js";
 ...
 */
export async function searchSessions(keyword, signal, limit = 100, fromIndex = 0, dateFrom = 0, dateTo = 0, scope = "visible", archivedIds = []) {
  ...
  const hits = [];
  // 搜索范围：visible（默认，排除归档）/ archived（仅归档）/ trash（回收站）
  let sessions;
  if (scope === "trash") {
    sessions = listTrash()
      .filter((t) => t.type === "session")
      .map((t) => ({ sessionId: t.name, cwd: "", path: path.join(t.entryDir, "data") }));
  } else {
    sessions = await listAllSessions();
    if (scope === "archived") {
      sessions = sessions.filter((s) => archivedIds.includes(s.sessionId));
    } else {
      // visible 默认：当前 workspace（左侧可见）+ 非归档；其他 workspace（如 /root）的会话左侧不可见，不混入
      sessions = sessions.filter(
        (s) => !archivedIds.includes(s.sessionId) && (s.cwd === WORKSPACE_ROOT || s.cwd.startsWith(WORKSPACE_ROOT + path.sep)),
      );
    }
  }
  ...
  for (let i = Math.min(fromIndex, sessions.length); i < sessions.length; i++) {
    ...
    try {
      ...
      if (frames.length === 0) {
        // 单帧场景（如插件写回的小文件）：整体解
        hits.push(...searchText(zstdDecompressSync(buf).toString("utf-8"), kw, s, 1, hits.length, limit, dateFrom, dateTo));
      ...
      for (const fr of frames) {
        ...
        hits.push(...searchText(text, kw, s, lineNo, hits.length, limit, dateFrom, dateTo));
        lineNo += text.split("\n").length;
        if (hits.length >= limit) break;
      }
    } catch (err) {
...
function searchText(text, kw, s, baseLineNo, hitStart, limit, dateFrom = 0, dateTo = 0) {
  const out = [];
