...
/** 用户选择「继续搜索」时的预算：不再中途截断（forceFull），直到搜完为止。 */
const SEARCH_FULL_BUDGET_MS = Infinity;
 ...
 * @param {boolean} [forceFull] true=跳过缓存、不做时间截断（用户选择「继续搜索」）
 * @returns {Promise<{hits: Array, partial: boolean}>} partial=true 表示超时截断
 */
export async function searchSessions(keyword, signal, limit = 100, forceFull = false) {
  ...
  if (!forceFull && ttlMs > 0 && searchCache.kw === kw && searchCache.hits && now - searchCache.at < ttlMs) {
    return {
  ...
  const budget = forceFull ? SEARCH_FULL_BUDGET_MS : SEARCH_TIME_BUDGET_MS;
  let partial = false;
