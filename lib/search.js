...
// 取消的搜索不缓存。
let searchCache = { at: 0, kw: "", hits: null };

/** 清空搜索缓存（释放内存按钮用）。 */
export function clearSearchCache() {
  searchCache = { at: 0, kw: "", hits: null };
}
 ...
 * @returns {Promise<Array<{sessionId, cwd, file, line, snippet, seq?}>>}
 */
export async function searchSessions(keyword, signal, limit = 100) {
  ...
  searchCache = { at: Date.now(), kw, hits };
  return hits;
}
