...
export async function searchSessions(keyword, signal, limit = 100, fromIndex = 0, dateFrom = 0, dateTo = 0, scope = "visible", archivedIds = []) {
  ...
  if (scope === "trash") {
    ...
    if (scope === "archived") sessions = sessions.filter((s) => archivedIds.includes(s.sessionId));
    else sessions = sessions.filter((s) => !archivedIds.includes(s.sessionId)); // visible 默认
  }
