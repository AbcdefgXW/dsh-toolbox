...
export async function embedQuery(cfg, keyword, topN = 20, signal, resolveSnippet) {
  ...
  // 相关度门槛（可配置 0-100，默认 50）：低于门槛的命中视为噪声（短泛词如「历史」语义过宽）
  // 全部低于门槛 → 降级关键词搜索（前端已有 fallback 提示链路）
  ...
  const minScore = (Number.isFinite(cfgScore) && cfgScore >= 0 && cfgScore <= 100 ? cfgScore : 50) / 100;
  const cfgTop = Number(cfg.embedTopN);
  const hitLimit = Number.isFinite(cfgTop) && cfgTop > 0 ? cfgTop : (topN > 0 ? topN : Infinity); // 0 = 不限制
  const filtered = scored.filter((h) => h.score >= minScore);
  if (filtered.length === 0) {
    return { ok: false, fallback: true, error: "语义匹配度过低（最高 " + scored[0].score.toFixed(2) + "），「" + kw.slice(0, 20) + "」更适合用关键词搜索，或换更具体的描述" };
  }
