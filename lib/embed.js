...
/** 语义查询：关键词 → 向量 → 与索引比对。 */
export async function embedQuery(cfg, keyword, topN = 20, signal) {
  ...
  const result = { ok: true, hits: scored.slice(0, topN).map((h) => ({ sessionId: h.sessionId, seq: h.seq, score: Number(h.score.toFixed(3)) })), total, cache: { cached: false, ttlSeconds: ttlSec, expiresInMs: ttlMs } };
