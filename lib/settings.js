...
export const TOOL_SWITCHES = [
  ...
  },
  {
    key: "embedSearch",
    label: "语义搜索",
    description: "在线 embedding 语义搜索（需配置下方 API Key；无 Key/API 失败自动降级为关键词搜索；默认关）",
    hot: false,
    default: false,
  },
  {
    ...
    description: "OpenAI 兼容端点，默认 SiliconFlow：https://api.siliconflow.cn/v1（也支持阿里云等兼容服务）",
    hot: false,
  ...
  },
  {
    ...
    description: "在对应平台（如 cloud.siliconflow.cn → API 密钥）生成的 Key",
    hot: false,
    default: "",
  },
  {
    key: "embedModel",
    label: "Embedding 模型",
    description: "如 BAAI/bge-m3（SiliconFlow）或 text-embedding-v3（阿里云）",
    hot: false,
    default: "BAAI/bge-m3",
  },
  {
...
/** 数值型开关单独处理。 */
const NUMBER_KEYS = new Set(["trashRetentionDays", "collapseUserThreshold", "scheduleInterval"]);

/** 文本型开关（提示语等）。 */
const STRING_KEYS = new Set(["schedulePrompt", "scheduleCron", "scheduleTarget", "scheduleCronTarget", "scheduleCronPrompt"]);
