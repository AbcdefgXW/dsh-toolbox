...
export const TOOL_SWITCHES = [
  {
    key: "scheduleTask",
    label: "定时心跳",
    description: "定时向主工作区会话注入心跳消息，唤醒守一执行巡检/汇报等任务（类似 OpenClaw 心跳模式）。⚠️ 会消耗 token；默认关；间隔与提示语见下方（改动 1 分钟内生效）",
    hot: false,
    default: false,
  },
  {
  ...
  },
  {
    key: "schedulePrompt",
    label: "心跳提示语",
    description: "注入给守一的心跳内容；{time} 会自动替换为当前时间",
    hot: false,
  ...
  },
  {
    key: "scheduleTarget",
    label: "心跳目标会话",
    description: "心跳注入到哪个会话：留空 = 主工作区根（默认）；指定 = 只注入该会话",
    hot: false,
    default: "",
  },
  {
...
/** 文本型开关（提示语等）。 */
const STRING_KEYS = new Set(["schedulePrompt", "scheduleCron", "scheduleTarget"]);
